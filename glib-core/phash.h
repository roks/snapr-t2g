...
template<class TKey, class TDat, class THashFunc>
int TPHash<TKey, TDat, THashFunc>::AddKey11(const int& BegTableN, const TKey& Key, bool& Found) {
  //int CurVals = __sync_fetch_and_add(&NumVals.Val, 1);
  //IAssertR(CurVals < Table.Len(), "Table must not be full");
...
template<class TKey, class TDat, class THashFunc>
int TPHash<TKey, TDat, THashFunc>::AddKey12(const int& BegTableN, const TKey& Key, bool& Found) {
  //int CurVals = __sync_fetch_and_add(&NumVals.Val, 1);
  //IAssertR(CurVals < Table.Len(), "Table must not be full");
...
template<class TKey, class TDat, class THashFunc>
int TPHash<TKey, TDat, THashFunc>::AddKey1(const TKey& Key, bool& Found) {
  const int Length = Table.Len();
...
template<class TKey, class TDat, class THashFunc>
int TPHash<TKey, TDat, THashFunc>::AddKey2(const int& BegTableN, const TKey& Key, bool& Found) {
  //const int Length = 12582917;
  const int Length = Table.Len();
  const int HashCd = abs(Key.GetSecHashCd());
...
template<class TKey, class TDat, class THashFunc>
int TPHash<TKey, TDat, THashFunc>::GetRndKeyId(TRnd& Rnd) const  {
  printf("*** ERROR *** TPHash<TKey, TDat, THashFunc>::GetRndKeyId called\n");
  return 0;
}

template<class TKey, class TDat, class THashFunc>
int TPHash<TKey, TDat, THashFunc>::GetRndKeyId(TRnd& Rnd, const double& EmptyFrac) {
  printf("*** ERROR *** TPHash<TKey, TDat, THashFunc>::GetRndKeyId called\n");
  return 0;
}
